...
description: 列表代码块参考文档：包括创建空列表、创建列表、追加列表项、检查列表中是否包含对象、求列表长度、列表是否为空？、随机选取列表项、选择列表中索引值对应的列表项、替换列表中索引值对应的列表项、删除列表中某项、将一个列表中所有项追加到另一个列表中、复制列表、是否为列表？、列表转置、列表转换为CSV行、列表转换为CSV表、CSV行转换为列表、CSV表转换为列表、在键值对中查找、分隔符拼接成文本、列表去重、列表赋值、列表比较。
---
...
* [*案例分享1：*去除列表重复项（**列表去重**）](#uniquekeylist)
* [*案例分享2：*将一个列表等于另一个列表（**列表赋值**）](#list_assign)
* [*案例分享3：*判断一个列表是否等于另一个列表（**列表比较**）](#list_compare)
* [*案例分享4：*清理列表（ClearList）](#list_clear)
