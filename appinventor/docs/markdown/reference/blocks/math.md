...
description: 数学代码块参考文档，包括：基础数字块,进制数字块,等于,不等于,大于,大于等于,小于,小于等于,加,减,乘,除,幂运算,随机整数,随机小数,设定随机数种子,最小值,最大值,平方根,绝对值,相反数,对数,e^,四舍五入,上取整,下取整,求模,余数,商,正弦,余弦,正切,反正弦,反余弦,反正切,反正切2,将弧度转换为度数,将度数转换为弧度,数字转变为小数形式,是否为数字,进制转换,按位与,按位或,按位异或，进制转换案例，文本转数字，数字转文本。
---
