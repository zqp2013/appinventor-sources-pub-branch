...
description: 字典代码块参考文档：包括什么情况下需要使用字典、创建空字典、创建字典、键值对、获取键的值、设置键的值、删除键的条目、获取键路径的值、设置键路径的值、获取键列表、获取值列表、键是否在字典中？、字典的长度、键值对列表转换为字典、字典转换为键值对列表、复制字典（深拷贝）、合并字典、在此层级遍历、遍历键路径列表、是否为字典？、字典遍历,遍历字典,获取键值列表的键（列表）。
---
